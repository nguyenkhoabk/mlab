<?php
class ModelSaleStudent extends Model {
	...
	public function getStudents($data = array()) {
		$sql = "SELECT *, CONCAT(c.lastname, ' ', c.firstname) AS name,
				CONCAT_WS(', ',a.address_1, a.city, z.name, ct.name) AS address
				FROM " . DB_PREFIX . "hocvien c
				LEFT JOIN ".DB_PREFIX . "address a
				ON c.hocvien_id = a.customer_id
				INNER JOIN ".DB_PREFIX . "country ct
	...
	public function getTotalStudents($data = array()) {
      	$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "hocvien";
